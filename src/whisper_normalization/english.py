"""
This code is adapted from the OpenAI Whisper repository:
...
class EnglishNumberNormalizer:
    ...
    - remove any commas
    - keep the suffixes such as:
        `1960s`, `274th`, `32nd`, etc.
    - spell out currency symbols after the number. e.g.
        `$20 million` -> `20000000 dollars`
        ...
        for prev, current, next in windowed([None] + words + [None], 3):
            ...
            if re.match(r"^\d+(\.\d+)?$", current_without_prefix):
                ...
                if next not in self.words and not next_is_numeric:
                    # apply special handling only
                    # if the next word can be numeric
                    if value is not None:
                        yield output(value)
                    yield output(current)
                elif current == "and":
    ...
    def __call__(self, s: str):
        ...
        s = " ".join(
            word
            for word in self.process_words(s.split())
            if word is not None
        )
...
class EnglishTextNormalizer:
    def __init__(self):
        ...
        self.replacers = {
            ...
            r"\bsr\b": "senior ",
            r"\besq\b": "esquire ",
            # prefect tenses, ideally it should be
            # any past participles, but it's harder..
            r"'d been\b": " had been",
            r"'s been\b": " has been",
            r"'d gone\b": " had gone",
            r"'s gone\b": " has gone",
    ...
    def __call__(self, s: str):
        ...
        # now remove prefix/suffix symbols that are
        # not preceded/followed by numbers
        s = re.sub(r"[.$¢€£]([^0-9])", r" \1", s)
        s = re.sub(r"([^0-9])%", r"\1 ", s)
